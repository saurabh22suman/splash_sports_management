...
class TestRLSMigration:
    ...
    async def test_migration_creates_rls_policies(self, db_engine):
        ...
        async with db_engine.begin() as conn:
            for table in TENANT_SCOPED_TABLES:
                ...
                assert row[2] == True or row[2] == 'PERMISSIVE', f"Policy on {table} is not permissive: {row[2]}"
    ...
    async def test_tenant_isolation_policy_structure(self, db_engine):
        ...
        async with db_engine.begin() as conn:
            ...
            """), {"id": tenant_a_id, "name": "Tenant A", "slug": "tenant-a", "status": "active", "email": "a@tenant.com"})
            ...
            """), {"id": str(uuid.uuid4()), "tenant_id": tenant_a_id, "email": "user@tenant-a.com", "hash": "hash", "name": "User A"})
            await conn.commit()
        ...
        async with db_engine.begin() as conn:
            ...
            assert 'current_setting' in row[3], f"WITH CHECK clause should use current_setting: {row[3]}"
    ...
    async def test_downgrade_removes_policies(self, db_engine):
        ...
        async with db_engine.begin() as conn:
            ...
            """))
            count = result.scalar()
            assert count == len(TENANT_SCOPED_TABLES), f"Expected {len(TENANT_SCOPED_TABLES)} policies, found {count}"
