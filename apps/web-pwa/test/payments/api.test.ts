...
import {
  ...
  listInvoices,
  refundInvoice,
} from "@splashh/api-client/payments";
...
describe("payments api", () => {
  ...
  it("createPaymentLink posts to /payments/invoices/:id/payment-link with Idempotency-Key", async () => {
    ...
    expect(api.post).toHaveBeenCalledWith("/payments/invoices/inv-1/payment-link", undefined, {
  ...
  it("listInvoices sends status and customer_id query params", async () => {
    ...
    expect(api.get).toHaveBeenCalledWith("/payments/invoices", {
      params: { status: "paid", customer_id: "cust-1" },
    });
  });
