import { Card, CardContent, CardFooter, CardHeader, CardTitle, EmptyState, ErrorState, LoadingSkeleton, Button, ArrowRight, cn } from "@splashh/ui";
      ...
      {!isLoading && !error && (data?.length ?? 0) > 0 && (
        <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {data!.map((f, index) => (
            <li key={f.id}>
              <Card className={cn(
                "h-full flex flex-col transition-shadow hover:shadow-volt-sm",
              )}>
                ...
                <CardContent className="flex-1 text-sm text-muted-foreground space-y-2">
                  <p>
                    {f.city}{f.state && `, ${f.state}`}{f.country && `, ${f.country}`}
                  </p>
                ...
                <CardFooter>
                  <Button asChild variant={index === 0 ? "default" : "outline"} className="w-full">
                    <Link to={`/book/facilities/${f.id}`} className="flex items-center justify-center gap-2">
                      View details
